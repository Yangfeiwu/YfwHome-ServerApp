...
class UserMessageHandler
{
	public static function handleMessage($client_id, $message_data, $db)
	{
		...
		{	
			case 'BIND':
			  LOG::OutLog("[UserMessageHandler]:","BIND....！\n"); //
				self::checkBind($client_id, $message_data);
			break;
			case 'UNBIND':
			 LOG::OutLog("[UserMessageHandler]:","UNBIND....！\n"); //
				self::checkUnbind($client_id);
			break;
			case 'CONTROL_DevStat':
			 LOG::OutLog("[UserMessageHandler]:","CONTROL_DevStat....！\n"); //
				self::ControlDevStat($client_id, $message_data);
			break;
			case 'QUERY_DevStat':
			LOG::OutLog("[UserMessageHandler]:","QUERY_DevStat....！\n"); //
				//self::queryPostureByDB($client_id, $message_data, $db);
			break;
			case 'QUERY_RECORD':
				LOG::OutLog("[UserMessageHandler]:","QUERY_RECORD....！\n"); //
				//self::queryRecord($client_id, $message_data, $db);
			break;
			default:
			
			LOG::OutLog("[UserMessageHandler]:","错误type".$message_data['type']."....！\n"); //
	...
	private static function checkBind($client_id, $message_data)
	{
	  ...
	  LOG::OutLog("[UserMessageHandler]:","checkBind....！\n"); //
		//当前PID+pid
        ...
        //	LoggerServer::log(Utils::INFO, "User[". $client_id ."]: ". Utlis::FAIL_BIND_PIDNULL_TEXT ."\n");
		  LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 绑定失败PID不正确\n"); //
		...
		if (count($content) != 1)
		{
        	...
        	//LoggerServer::log(Utils::INFO, "Server: ". Utlis::FAIL_ILLEGAL_INSTRUIONS_TEXT ." Length of BIND instruction is not 1.\n");
			  LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 绑定失败，content中不只有1个信息.\n"); //
			return false;
		}
		...
		if (self::isBedOnline($PID)) {
			...
			self::sendServerFeedback(Utils::SUCCESS_BIND_CODE, Utils::SUCCESS_BIND_TAG);
			//info
        	//LoggerServer::log(Utils::INFO, "Server: ". "User[". $client_id ."]: ". Utils::SUCCESS_BIND_TAG ."\n");
			LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 绑定成功.\n"); //
			//增加指令发送标志位
			//$_SESSION['sendTag'] = false;
			return true;
		}else{
			//绑定的PID设备不在线 
			self::sendServerFeedback(Utils::FAIL_BIND_OFFLINE_CODE, Utils::FAIL_BIND_OFFLINE_TAG);
			//info
        	//LoggerServer::log(Utils::INFO, "User[". $client_id ."]: ". Utils::FAIL_BIND_OFFLINE_TAG ."\n");
			LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 绑定失败，设备不在线.\n"); //
			return false;
		}
	...
	private static function checkUnbind($client_id){
	  ...
	  LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 解绑成功.\n"); //
	}
	 ...
	 * @return bool
	 */
	private static function ControlDevStat($client_id, $message_data)
	{
		//info
      //  LoggerServer::log(Utils::INFO, "User[". $client_id ."]: send control_posture to Bed...\n");
		  LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 服务器接收到控制指令.\n"); //
		  
		$angle = intval(reset($message_data['content']));
		
		$pos = key($message_data['content']);
		
		LOG::OutLog("[UserMessageHandler]","User_". $client_id .": pos值：".$pos."\n"); //
	switch ($pos){
		
			case 'other':
				 LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 控制指令属于MSG类，值".$angle.".\n"); //
				$new_package = array('length' => 3, 'type' => Utils::CONTROL_MSGCLASS, 'pos' => 0, 'angle' => $angle);
				break;
				
			case 'msg':
				$strdata=$message_data['content']['msgdata'];
				 LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 控制指令属于other类，值".$angle.".\n"); //
				 
				// $DATA="1234567891"; //接收的消息文本内容
				 $datalen= strlen($strdata); //消息字符串长度
				 
				$new_package = array('length' =>  $datalen+1, 'type' => Utils::CONTROL_MSGCLASS, 'data' => $strdata);
				break;		
						
			default:
			 LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 不支持该指令.\n"); //
			return false;
...
*/
		...
		switch (Gateway::getSession($bed_id)['port']) {
			case 8281:
			 	LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 目标设备".$bed_id.",目标设备在8281端口.\n"); //
		...
		//	case 8280:
			...
			default:
				//info
        	  LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 目标端口号不存在.\n"); //	
			  break;
		}
		LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 成功发送到设备.\n"); //
	/*	
		//info
      //  LoggerServer::log(Utils::INFO, "Server: ". Utils::SUCCESS_CONTROLPOSTURE_TAG ."\n");
	    LOG::OutLog("[UserMessageHandler]","User_". $client_id .": 控制指令发送成功.\n"); //
 ...
 LOG::OutLog("[UserMessageHandler]","User_". $client_id .":  send query_posture to Bed.\n"); //	
		//得到设备pid
		$bed_id = self::getBedID($client_id);
		if (empty($bed_id)) {
			return false;
	...
	private static function getBedID($user_id){
       ...
       // LoggerServer::log(Utils::INFO, "Server: ". Utils::FAIL_OFFLINE_TAG ." \n");
	    LOG::OutLog("[UserMessageHandler]","User_". $client_id ."Server: ". Utils::FAIL_OFFLINE_TAG ." \n"); //	
		return null;
	}
